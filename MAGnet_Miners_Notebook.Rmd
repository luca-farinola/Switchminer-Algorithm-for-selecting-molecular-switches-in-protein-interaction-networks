...
## Validating Biomarkers for Type II Diabetes
...
```{r panel}
marker_panel <- t(gxData.tpm[biomarkers$ensembl_gene_id,])
colnames(marker_panel) <- biomarkers$hgnc_symbol
marker_panel <- marker_panel[ , colSums(is.na(marker_panel))==0]

dcm_assay <- marker_panel[sampleInfo$etiology == 'DCM',]
dcm_assay <- dcm_assay[,c('RAPGEF3','KCNJ11','TNNI3','CAMK2D','CDKAL1','ADCY9','ADRA1B')]
nf_assay <- marker_panel[sampleInfo$etiology == 'NF',]

ground_truth <- sampleInfo[sampleInfo$etiology=='DCM',]
y <- ground_truth[,c('sample_name','Diabetes')]

panel <- pheatmap(dcm_assay, scale = 'column', treeheight_col = 0, show_rownames = TRUE)
y_pred <- cutree(panel$tree_row, k=2)
